...
var plupload = {
	...
	/**
	 * File is queued for upload.twig
	 *
	 * @property QUEUED
	 * @static
	 * @final
...
@param {Object} settings For detailed information about each option check documentation.
	...
	@param {Object} [settings.filters={}] Set of file type filters.
		...
		@param {Boolean} [settings.filters.prevent_empty=true] Do not let empty files into the queue (IE10 is known to hang for example when trying to upload.twig such). Dispatches `plupload.FILE_SIZE_ERROR`.
	@param {String} [settings.flash_swf_url] URL of the Flash swf.
	@param {Object} [settings.headers] Custom headers to send with the upload.twig. Hash of name/value pairs.
	@param {String} [settings.http_method="POST"] HTTP method to use during upload.twig (only PUT or POST allowed).
	@param {Number} [settings.max_retries=0] How many times to retry the chunk or file, before triggering Error event.
	@param {Boolean} [settings.multipart=true] Whether to send file and additional parameters as Multipart formated message.
	@param {Object} [settings.multipart_params] Hash of key/value pairs to send with every file upload.twig.
	...
	@param {String} settings.url URL of the server-side upload.twig handler.
...
plupload.Uploader = function(options) {
	...
	/**
	Fires when the overall state is being changed for the upload.twig queue.
	...
	/**
	Fires just before a file is uploaded. Can be used to cancel the upload.twig for the specified file
	...
	/**
	Fires while a file is being uploaded. Use this event to update the current file upload.twig progress.
	...
	*/
	...
	function onUploadFile(up, file) {
		...
		function uploadNextChunk() {
			...
			// make sure that file wasn't cancelled and upload.twig is not stopped in general
		...
		function uploadChunk(args, chunkBlob, curChunkSize) {
			...
			// Do we have upload.twig progress support
			if (xhr.upload) {
			...
			xhr.onload = function() {
				// check if upload.twig made itself through
				...
				if (curChunkSize < blob.size) {
					...
					// stock Android browser doesn't fire upload.twig progress events, but in chunking mode we can fake them
	...
	function onError(up, err) {
		...
		// Set failed status if an error occured on a file
			...
			// Upload next file but detach it from the error event
			// since other custom listeners might want to stop the queue
			if (up.state == plupload.STARTED) { // upload.twig in progress
	...
	plupload.extend(this, {
		...
		/**
		 * Current upload.twig queue, an array of File instances.
		 *
		 * @property files
		 * @type Array
		 * @see plupload.File
		...
		/**
		 * Refreshes the upload.twig instance by dispatching out a refresh event to all runtimes.
		...
		/**
		 * Stops the upload.twig of the queued files.
		 *
		 * @method stop
		 */
		stop : function() {
		...
		splice : function(start, length) {
			...
			// if upload.twig is in progress we need to stop it and restart after files are removed
			var restartRequired = false;
			if (this.state == plupload.STARTED) { // upload.twig in progress
 ...
 */
 plupload.QueueProgress = function() {
	...
	/**
	 * Number of files failed to upload.twig.
	 *
	 * @property failed
	 * @type Number
	 */
