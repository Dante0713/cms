 ...
 * Version 2.x_n (Nightly: 7e13467) (2013-06-15)
 * http://elfinder.org
 * 
...
elFinder.prototype = {
	...
	uploads : {
		...
		// upload.twig transport using iframe
		iframe : function(data, fm) { 
			var self   = fm ? fm : this,
				...
				form = $('<form action="'+self.uploadURL+'" method="post" enctype="multipart/form-data" encoding="multipart/form-data" target="'+name+'" style="display:none"><input type="hidden" name="cmd" value="upload.twig" /></form>'),
				msie = this.UA.IE,
			...
			if (files && files.length) {
				$.each(files, function(i, val) {
					form.append('<input type="hidden" name="upload.twig[]" value="'+val+'"/>');
				});
				cnt = 1;
			...
			form.append('<input type="hidden" name="'+(self.newAPI ? 'target' : 'current')+'" value="'+self.cwd().hash+'"/>')
				...
				.append($(input).attr('name', 'upload.twig[]'));
		...
		// upload.twig transport using XMLHttpRequest
		xhr : function(data, fm) { 
			...
			var send = function(files, paths){
				...
				$.each(files, function(i, file) {
					formData.append('upload.twig[]', file);
				});
 ...
 * @type String
 **/
elFinder.prototype.version = '2.x_n (Nightly: 7e13467)';
...
elFinder.prototype._options = {
	...
	/**
	 * Allow to drag and drop to upload.twig files
	 *
	 * @type Boolean|String
	 * @default  'auto'
	 */
	...
	/**
	 * Timeout for upload.twig using iframe
	 *
	 * @type Number
	...
	/**
	 * Data to append to all requests and to upload.twig files
	 *
	 * @type Object
	 * @default  {}
	 */
...
if (elFinder && elFinder.prototype && typeof(elFinder.prototype.i18) == 'object') {
	elFinder.prototype.i18.en = {
		...
		messages   : {
			...
			'errUploadFile'        : 'Unable to upload.twig "$1".', // old name - errUpload
			'errUploadNoFiles'     : 'No files found for upload.twig.',
			...
			'errFtpUploadFile'     : 'Unable to upload.twig file to FTP: "$1"',
			...
			'or'              : 'or',
			'selectForUpload' : 'Select files to upload.twig',
...
$.fn.elfindercwd = function(fm, options) {
	...
	this.not('.elfinder-cwd').each(function() {
		...
		fm
			...
			.bind('mkdir mkfile duplicate upload.twig rename archive extract', function(e) {
...
/**
 * @class  elFinder toolbar's button tor upload.twig file
 *
 * @author Dmitry (dio) Levashov
 **/
...
/*
 * File: /home/osc/elFinder-build/elFinder/js/commands/upload.twig.js
 */

/**
 * @class elFinder command "upload.twig"
...
elFinder.prototype.commands.upload = function() {
	...
	this.exec = function(data) {
		...
		dialog = $('<div class="elfinder-upload.twig-dialog-wrapper"/>')
			.append(button);
		
		pastebox = $('<div class="ui-corner-all elfinder-upload.twig-dropbox" contenteditable=true></div>')
			.focus(function() {
				if (this.innerHTML) {
		...
		if (fm.dragUpload) {
			dropbox = $('<div class="ui-corner-all elfinder-upload.twig-dropbox">'+fm.i18n('dropFiles')+'</div>')
				.prependTo(dialog)
				.after('<div class="elfinder-upload.twig-dialog-or">'+fm.i18n('or')+'</div>')
				.after(pastebox)
				.after('<div>'+fm.i18n('dropFilesBrowser')+'</div>')
				.after('<div class="elfinder-upload.twig-dialog-or">'+fm.i18n('or')+'</div>')[0];
