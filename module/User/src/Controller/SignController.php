...
use ZendDeveloperTools\View\Helper\Time;
    ...
    public function registerAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                $data = $form->getData();
                // 檢查帳號是否重複
                $res =  $em->getRepository('Base\Entity\User')->findOneBy(array(
                    'username' => $form->get('username')->getValue()));
                ...
                if ($is_ok) {
                    ...
                    $userRes->setType('A');
                    $userRes->setStatus('1');
                    $userRes->setChineseName($data['name']);
                    $userRes->setNickname($data['nickname']);
                    $userRes->setCreateTime(new \DateTime('now'));
                    $userRes->setEmail($data['email']);
                    $userRes->setGender($data['gender']);
...
//
                    ...
                    $template->setSender('@sub.mdn.tw');
                    $template->addFrom('jackmel030@gmail.com', '會員通知');
                    ...
                    $this->getFlashMessenger()->addInfoMessage($data['name'] . ' 註冊成功！！');
